...
def main(country):
    ...
    # Make CLIMADA Exposure 
    exp_ivm = Exposures(gdf_bem_subcomps)
    exp_ivm.gdf.rename({'valhum': 'value'}, axis=1, inplace=True)
    exp_ivm.value_unit = 'Pop. count'
    exp_ivm.gdf['longitude'] = exp_ivm.gdf.geometry.x
    exp_ivm.gdf['latitude'] = exp_ivm.gdf.geometry.y
    exp_ivm.gdf = exp_ivm.gdf[~np.isnan(
        exp_ivm.gdf.latitude)]  # drop nan centroids
    ...
    # Make CLIMADA Exposure
    exp_cima = Exposures(gdf_bem_subcomps)
    exp_cima.gdf.rename({'valhum': 'value'}, axis=1, inplace=True)
    exp_cima.value_unit = 'Pop. count'
    exp_cima.gdf['longitude'] = exp_cima.gdf.geometry.x
    exp_cima.gdf['latitude'] = exp_cima.gdf.geometry.y
    exp_cima.gdf = exp_cima.gdf[~np.isnan(
        exp_cima.gdf.latitude)]  # drop nan centroids
    ...
    def calculate_impacts_and_return_df(exp, impf_set, haz, threshold):
        ...
        total_population = exp.gdf['value'].sum()
    ...
    file_path = disp_code_base.joinpath(f'{country}_displacement.csv')
    ...
    # Save the combined DataFrame to CSV
    file_path = disp_code_base.joinpath(f'{country}_displacement_10cm.csv')
    print(f"Saving file to: {file_path}")
